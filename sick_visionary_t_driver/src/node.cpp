...
//        sensor_msgs::CameraInfoPtr ciPtr;
	std_msgs::Header header,header_laser;
	header.stamp = ros::Time::now();
	header.frame_id = g_frame_id;
    header_laser.stamp = ros::Time::now();// changed 
    header_laser.frame_id ="camera_rotate";// changed
...
//	        std::cout<<"w:"<<width<<"h:"<<height<<std::endl;
		...
		scan_msg.angle_min = -34.5 * (3.14/180); //define pi
	        scan_msg.angle_max = 34.5 * (3.14/180);
            	scan_msg.angle_increment = 69.0/width * (3.14/180);
            	scan_msg.range_min = 10.0/1000; //m
            	scan_msg.range_max = 10.0; //m
		scan_msg.header = header_laser; 
...
//                   unsigned short num = mat.at<unsigned short>(j,i)/cos(56.0*3.14/360.0-j*56.0*3.14/180.0/height)/cos(69.0*3.14/360.0-i*69.0*3.14/180.0/width);
...
//    std::cout<<"here blok "<<i<<" "<<scan_msg.ranges.size()<<std::endl;
	    ...
	    scan_msg.ranges[i]=(float)min/1000.0;
